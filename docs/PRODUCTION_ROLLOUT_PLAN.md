...
**Document status:** Proposed rollout baseline  
...
目前 repository 实际包含三个不同成熟度的系统：

1. **数据看板**：Python pipeline 生成 CSV、SQLite、JSON 和 GeoJSON，React 前端读取静态数据。
2. **身份系统**：Express、PostgreSQL、session、CSRF、邮件验证及独立 email worker。
3. **原型功能**：News 和 Community 目前使用 mock data、`localStorage` 和 IndexedDB，不是共享的 production backend。

因此，页面已经出现在 UI 中，并不代表对应功能已经具备 production persistence、authorization 或 operational support。
...
相关文件：[server/README.md](../server/README.md)、[server/src/app.ts](../server/src/app.ts)
...
当前 News 只使用 mock data 和 `localStorage`，不能直接作为 production publishing system。
...
当前 prototype：[newsStore.js](../src/services/newsStore.js)、[adminNewsService.js](../src/services/adminNewsService.js)
