...
### Stage 3C implementation note: deterministic comparability gate

Implemented in `supabase/functions/ai-chat/comparabilityGate.ts`. The gate is deterministic only: no Gemini, no embeddings, no live Supabase, and no external web calls. It runs after request validation and the current internal routing/entity inference, logs only safe metadata (`decision`, blocked/allowed operations, disclosure count), and does not change the public Stage 1A response contract.
...
SDG downgrade:
- Repository metadata has SDG mappings but no target fields. Questions asking for SDG "progress" return `DOWNGRADE`; only coverage/mapping explanations are safe, with disclosure that progress-to-target cannot be calculated.

District staleness behavior:
...
- Ambiguous district names return `NEEDS_CLARIFICATION`; unknown districts return `REJECT` and must not be guessed.
