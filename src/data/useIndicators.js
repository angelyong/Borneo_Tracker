...
// BT-18/BT-19 resilience history. Like the source registry this is an Option A
// auxiliary artifact outside useIntegrity's immutable six-file scope, and it is
// generated by the refresh workflow rather than committed with feature code —
// so "not published yet" (404) is an expected state, not a fault. Callers get
// `unavailable` and are expected to render nothing rather than an error.
export function useResilienceHistory() {
  const [state, setState] = useState({
    data: null,
    loading: true,
    error: null,
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const response = await fetch('/data/resilience_history.json');
        if (response.status === 404) {
          if (!ignore) setState({ data: null, loading: false, error: null, unavailable: true, generatedAt: null });
          return;
        }
        if (!response.ok) throw new Error(`Failed to load resilience history (${response.status})`);
        const payload = await response.json();
        if (!ignore) {
          setState({
            data: payload,
            loading: false,
            error: null,
            unavailable: !payload?.territories,
            generatedAt: payload?.generatedAt ?? null,
          });
        }
      } catch (error) {
        if (!ignore) {
          setState({ data: null, loading: false, error: error.message, unavailable: true, generatedAt: null });
        }
      }
    }
